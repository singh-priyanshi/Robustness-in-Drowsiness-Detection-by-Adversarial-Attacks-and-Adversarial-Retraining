...
The code for implementing adversarial attacks and retraining models can be found in the "Code" folder. The code is organized into subfolders based on the attack technique used, such as "FGSM," "PGD," "Auto-PGD," "DDN," "DeepFool," and "Patch."

### Code Usage

1. Each attack folder contains a script (e.g., `fgsm_attack.py`) that implements the respective adversarial attack on both Mobilenet V2 and Custom CNN models.
2. The `retrain_model.py` script is used for retraining the models with the adversarial data.
3. Results and visualizations are generated by running the appropriate analysis scripts (e.g., `analyze_results.py`).
